...
const lidarPosition = new THREE.Vector3(0, 1, 0);
const lidarRange = 15;
const horizontalResolution = 180;
const verticalResolution = 50;
const horizontalFov = Math.PI * 2;
const verticalFov = Math.PI;
const scanFrequency = 5;
...
const beamDivergenceAngle = THREE.MathUtils.degToRad(5.2);
const pulseDurationNanoSeconds = 0.1;
...
function simulateLidarScan() {
    ...
    // Raycasting loop (same as before)
...
function animate(time) {
    ...
    if (deltaTime >= (1 / scanFrequency)) {
        ...
        // Update beams (will clear if needed)
